...
if ( is_admin() ) {
	...
	$this->show_me = '<pre>$media_queries_list: '.print_r($media_queries_list, true). '</pre>' .
			...
			function json_format_ua($icon, $item, $val = false){
				...
				if ($val){
					$json.= '<span class=\"his-val\">'.htmlentities($val).'</span>';
				}
				$json.= '</span>"}';
				return $json;
			}
			...
			function getRevisions($summary = false) {
				...
				foreach ($revs as $rev) {
                    ...
                    // remove any HTML from history val, which should be CSS
                    preg_match('/<span class="his-val">(.+?)<\/span>/', $user_action, $history_val_match);
                    if ($history_val_match){
				...
				if( $_GET['page'] == $this->detachedpreviewpage ) {
                    ...
                    <div id="tvr" class="wrap tvr-wrap '.$ui_class.'">
                        ...
                        <div id="preview-standalone">';
                            ...
                            class="detached-alt-controls detached-advanced-wizard"></div>
			...
			*/
			...
			function add_js() {
				...
				if (!empty($this->preferences['active_events'])){
				    ...
				    //wp_die('active_events' . $this->preferences['active_events']);

					wp_localize_script( 'mt_animation_events', 'MT_Events_Data',
						json_decode($this->preferences['active_events'], true) );
			...
			function add_body_classes($classes){
				...
				if ( isset( $post ) ) {
					...
					if (!empty($this->preferences['insert_custom_field_classes'])){
					    $custom_classes = get_post_meta($post->ID, 'my_body_classes', true);
					    if ($custom_classes){
						    $custom_classes_array = preg_split("/\s+/", trim($custom_classes));
						    $classes = array_merge($classes, $custom_classes_array);
					    }
					    //wp_die('get_post_meta: ' . get_post_meta($post->ID, 'my_body_classes', true) );
					}
